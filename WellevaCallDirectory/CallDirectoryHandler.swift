...
class CallDirectoryHandler: CXCallDirectoryProvider {
    ...
    override func beginRequest(with context: CXCallDirectoryExtensionContext) {
        context.delegate = self
        addScamNumbers(to: context)
        context.completeRequest()
    }

    private func addScamNumbers(to context: CXCallDirectoryExtensionContext) {
        let numberStrings = ScamNumberStore.loadNumbers()

        let numbers: [CXCallDirectoryPhoneNumber] = numberStrings
...
extension CallDirectoryHandler: CXCallDirectoryExtensionContextDelegate {
    func requestFailed(for extensionContext: CXCallDirectoryExtensionContext, withError error: Error) {
        print("Call Directory extension failed: \(error)")
    }
}
